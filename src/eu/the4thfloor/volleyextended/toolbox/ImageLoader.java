 ...
 * The simple way to use this class is to call {@link ImageLoader#get(String, ImageListener)} and to pass in the default image listener provided by
...
public class ImageLoader {
   ...
   */
  public ImageLoader(final RequestQueue queue, final ImageCache imageCache) {
   ...
   */
  public ImageContainer get(final String requestUrl, final ImageListener imageListener, final int maxWidth, final int maxHeight) {
    ...
    final Bitmap cachedBitmap = this.mCache.getBitmap(cacheKey);
    if (cachedBitmap != null) {
      // Return the cached bitmap.
      final ImageContainer container = new ImageContainer(cachedBitmap, requestUrl, null, null);
      imageListener.onResponse(container, true);
      return container;
    }

    // The bitmap did not exist in the cache, fetch it!
    final ImageContainer imageContainer = new ImageContainer(null, requestUrl, cacheKey, imageListener);

    // Update the caller to let them know that they should use the default bitmap.
    imageListener.onResponse(imageContainer, true);

    // Check to see if a request is already in-flight.
    final BatchedImageRequest request = this.mInFlightRequests.get(cacheKey);
    if (request != null) {
    ...
    final Request<?> newRequest = new ImageRequest(requestUrl, new Listener<Bitmap>() {
      ...
      @Override
      public void onResponse(final Bitmap response, final boolean hasChanged) {
   ...
   */
  private void onGetImageSuccess(final String cacheKey, final Bitmap response) {
  ...
  public class ImageContainer {
     ...
     */
    public ImageContainer(final Bitmap bitmap, final String requestUrl, final String cacheKey, final ImageListener listener) {
    ...
    public void cancelRequest() {
      ...
      BatchedImageRequest request = ImageLoader.this.mInFlightRequests.get(this.mCacheKey);
      if (request != null) {
        final boolean canceled = request.removeContainerAndCancelIfNecessary(this);
        if (canceled) {
  ...
  private class BatchedImageRequest {
     ...
     */
    public BatchedImageRequest(final Request<?> request, final ImageContainer container) {
     ...
     * the request.
     */
    public void addContainer(final ImageContainer container) {
     ...
     */
    public boolean removeContainerAndCancelIfNecessary(final ImageContainer container) {
    ...
    if (this.mRunnable == null) {
      this.mRunnable = new Runnable() {
        ...
        public void run() {
          ...
          for (final BatchedImageRequest bir : ImageLoader.this.mBatchedResponses.values()) {
            for (final ImageContainer container : bir.mContainers) {
